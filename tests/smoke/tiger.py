"""
Pure-Python Tiger-192 hash for the smoke-test harness.

Tiger is the hash algorithm ADC uses for client identifiers (CID = Tiger(PID))
and the password challenge response (PAS = Tiger(password || salt)). Modern
Python crypto libraries (hashlib, pycryptodome, cryptography) no longer ship
Tiger - it predates SHA-2 and is rarely used outside ADC, so it has been
deprecated and removed from those packages over time. Vendoring the algorithm
here keeps the test harness self-contained and avoids pinning a deprecated
package version that would not build on current Python.

The S-box constants below are taken verbatim from `adclib/tiger.cpp`, which
is what the hub itself uses. Match-on-output is the implicit correctness
check - every BINF/PAS frame the smoke tests construct must verify against
the hub's own implementation, so any discrepancy here surfaces as a hub
"invalid PID" or "invalid password" rejection.

Reference: https://www.cs.technion.ac.il/~biham/Reports/Tiger/

Variant: Tiger-192 (24-byte / 192-bit output), 0x01 padding (the original
Tiger spec, not the later Tiger2 variant which uses 0x80 padding).
"""
...
# -----------------------------------------------------------------------------
# S-box: 4 sub-tables of 256 64-bit values each. Indexed as
# T[(0..3) * 256 + (0..255)] in the round function below.
# -----------------------------------------------------------------------------
_T = [
...
assert len(_T) == 1024, f"Tiger S-box must have 1024 entries, got {len(_T)}"

# Initial Tiger state (the chaining variables).
_INITIAL_STATE = (
    0x0123456789ABCDEF,
    0xFEDCBA9876543210,
    0xF096A5B4C3B2E187,
...
def _round(a, b, c, x, mul):
    """One round of the Tiger compression function.

    Implements the C macro `round(a, b, c, x, mul)` from adclib/tiger.cpp:
    XORs c with x, subtracts an S-box-derived value from a, adds another
    S-box-derived value to b, multiplies b by mul. Returns updated (a, b, c).
    """
    c = (c ^ x) & _MASK64
    a = (a - (
        _T[(c) & 0xff] ^
...
def _pass(a, b, c, x, mul):
    """One pass: 8 rounds with the role of c (target of XOR) cycling
    through (a, b, c). Mirrors the C macro `pass(a, b, c, mul)`."""
...
def _key_schedule(x):
    """The key schedule that runs between passes 2/3 and 3/3."""
    x0, x1, x2, x3, x4, x5, x6, x7 = x
    x0 = (x0 - (x7 ^ 0xA5A5A5A5A5A5A5A5)) & _MASK64
    x1 = (x1 ^ x0) & _MASK64
    x2 = (x2 + x1) & _MASK64
    x3 = (x3 - (x2 ^ ((((~x1) & _MASK64) << 19) & _MASK64))) & _MASK64
    x4 = (x4 ^ x3) & _MASK64
    x5 = (x5 + x4) & _MASK64
    x6 = (x6 - (x5 ^ (((~x4) & _MASK64) >> 23))) & _MASK64
    x7 = (x7 ^ x6) & _MASK64
    x0 = (x0 + x7) & _MASK64
    x1 = (x1 - (x0 ^ ((((~x7) & _MASK64) << 19) & _MASK64))) & _MASK64
    x2 = (x2 ^ x1) & _MASK64
    x3 = (x3 + x2) & _MASK64
    x4 = (x4 - (x3 ^ (((~x2) & _MASK64) >> 23))) & _MASK64
    x5 = (x5 ^ x4) & _MASK64
    x6 = (x6 + x5) & _MASK64
    x7 = (x7 - (x6 ^ 0x0123456789ABCDEF)) & _MASK64
    return [x0, x1, x2, x3, x4, x5, x6, x7]


def _compress(state, block):
    """Process one 64-byte block, returning the updated state tuple."""
    x = list(struct.unpack("<8Q", block))
    a, b, c = state
    aa, bb, cc = a, b, c

    for pass_no in range(_PASSES):
        if pass_no != 0:
            x = _key_schedule(x)
        mul = 5 if pass_no == 0 else (7 if pass_no == 1 else 9)
        a, b, c = _pass(a, b, c, x, mul)
        # End-of-pass rotation from the C macro: tmpa=a; a=c; c=b; b=tmpa
        a, b, c = c, a, b

    return ((a ^ aa) & _MASK64, (b - bb) & _MASK64, (c + cc) & _MASK64)


class TigerHash:
    """Tiger-192 hasher. Use update()+digest() or just call the module-level
    tiger() function for a one-shot hash."""
    ...
    def update(self, data):
        ...
        while len(self._buffer) >= _BLOCK_SIZE:
            block = bytes(self._buffer[:_BLOCK_SIZE])
            del self._buffer[:_BLOCK_SIZE]
            self._state = _compress(self._state, block)

    def digest(self):
        # Snapshot state and buffer so digest() is non-destructive.
        state = self._state
        buf = bytearray(self._buffer)
        bit_length = self._length * 8

        # Tiger-192 padding: append 0x01, then zero-pad up to a full block
        # leaving 8 bytes for the bit-length suffix (little-endian).
        buf.append(0x01)
        if len(buf) > _BLOCK_SIZE - 8:
            buf.extend(b"\x00" * (_BLOCK_SIZE - len(buf)))
            state = _compress(state, bytes(buf))
            buf = bytearray()
        buf.extend(b"\x00" * (_BLOCK_SIZE - 8 - len(buf)))
        buf.extend(struct.pack("<Q", bit_length))
        state = _compress(state, bytes(buf))
        return struct.pack("<3Q", *state)


def tiger(data):
    """One-shot Tiger-192 hash of `data` (bytes), returns 24 bytes."""
    return TigerHash(data).digest()


# -----------------------------------------------------------------------------
# Self-test on import: catch any S-box / port mistakes immediately rather
# than letting them surface as opaque "hub rejected our BINF" failures.
# -----------------------------------------------------------------------------

_TEST_VECTORS = [
    # Standard Tiger-192 test vectors.
...
def _self_test():
    for data, expected_hex in _TEST_VECTORS:
        got = tiger(data).hex().upper()
        if got != expected_hex:
            raise RuntimeError(
                f"tiger.py self-test FAILED for input {data!r}: "
